...
export function Plan2D({ input }: { input: ProjectInput }) {
  ...
  const pxL = L * scale;
  const pxW = W * scale;
  const displayGapMm = input.board.gapMm ?? 0;
  const pitchM = (input.board.widthMm + displayGapMm) / 1000;
  ...
  return (
    <div className="visual-card">
      <div className="visual-title"><span>Vue de dessus</span><code>IB-TERR-UI-2D-007</code></div>
